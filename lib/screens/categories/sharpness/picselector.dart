...
import 'package:bigbrain/screens/categories/sharpness/picturemode.dart';
...
class _PicSelectorState extends State<PicSelector> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: SizedBox(
          width: (sqrt(MediaQuery.of(context).size.width) *
                  sqrt(MediaQuery.of(context).size.height)) *
              0.8,
          child: GridView.count(
            padding: EdgeInsets.symmetric(
                horizontal: (sqrt(MediaQuery.of(context).size.width) *
                        sqrt(MediaQuery.of(context).size.height)) /
                    24),
            physics: const BouncingScrollPhysics(),
            shrinkWrap: true,
            crossAxisCount: 2,
            crossAxisSpacing: (sqrt(MediaQuery.of(context).size.width) *
                    sqrt(MediaQuery.of(context).size.height)) /
                48,
            mainAxisSpacing: (sqrt(MediaQuery.of(context).size.width) *
                    sqrt(MediaQuery.of(context).size.height)) /
                48,
            children: <InkWell>[
              for (int i = 1; i < 5; i++)
                    ...
                    Navigator.of(context)
                        .push(MyRoute(
                            builder: (BuildContext context) =>
                                PictureMode(i, widget._displayAd)))
                        .then((value) {
